...
public class PagingHandler {
	...
	private boolean prev, next; // 앞뒤 단위로 이동하는 버튼의 생성 여부
	private PageVO pgvo;
	...
	public PagingHandler(int totalCount, PageVO pgvo) {
		...
		this.pgvo = pgvo;
		// 17번(pageIndex)을 클릭했음
		// 17/10.0->1.7 -> ceil(1.7) - > 2.0 ->int(2.0) ->2=? 2* 10 -> 20(마지막 번호)
		this.lastPageIndex = (int)(Math.ceil(pgvo.getPageIndex()/10.0)) * 10;
		logger.info("첫번째 lastPage: " +lastPageIndex);
		...
		//로거는 테스트 이후에 삭제
		logger.info(">>> ? totalcount" + this.totalCount + "/firstpage" + this.firstPageIndex + "/"+ pgvo.getPageIndex()
				+ "/lastpage" + this.lastPageIndex + "/range" + pgvo.getRange() + "/keyword" + pgvo.getKeyword() + "/");
	}
